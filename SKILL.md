...
description: >
  Agnes AI 全模态生成技能。覆盖文本生成(agnes-2.0-flash)、图片理解/Vision(agnes-2.0-flash)、
  文生图(agnes-image-2.1-flash)、图生图、文生视频、图生视频、关键帧动画、自动中译英、异步轮询。
  兼容 WorkBuddy、Codex、Claude Code、OpenClaw、Cursor、Windsurf 等所有支持 Agent Skills 的框架。
  当用户需要 AI 生成文本/图片/视频，或提到 "Agnes"、"agnes"、"AI 画图"、"AI 生成视频"、
  "AI 图片"、"文生图"、"图生图"、"文生视频"、"关键帧动画"、"用 Agnes 生成" 时触发。
...
- **图片理解/Vision**: `agnes-2.0-flash`（OpenAI Vision API 兼容，可分析图片内容）
...
| 框架 | 安装方式 |
|------|---------|
| **WorkBuddy** | Skill 市场搜索 "agnes-multimodal" 或本地导入 `.skill` 文件 |
| **Codex / Claude Code / OpenClaw 等** | `npx skills add USERNAME/agnes-multimodal` |
| **Cursor / Windsurf 等** | 手动克隆到 `~/.agent/skills/` 目录 |
...
1. 确认 API Key 已配置：
   ...
   - 支持的环境变量：`AGNES_API_KEY` / `AGNES_API_TOKEN` / `APIHUB_AGNES_API_KEY`
...
# 图片理解 / Vision（用 agnes-2.0-flash 分析图片内容）
...
1. 接收用户的中文/英文提示词
2. 若含中文且未指定 `--no-translate`，自动调用 `agnes-2.0-flash` 翻译为英文
...
- 快速测试用 `--num_frames 81`（约 3.4 秒）

### 图片理解 (Vision) 流程

`agnes-2.0-flash` 支持 OpenAI Vision API 格式的多模态输入，可分析图片内容并返回文字描述。这是"给不具备视觉能力的模型提供眼睛"的桥接方案。
